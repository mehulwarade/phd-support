...
do
	...
	jobstate_file=$i'n_i'$individual'_30k-jobstate.log'
	mysql_table=$i'n_i'$individual'_30k'
	...
	mysql -uroot -pz phd -ss -e "(select 'timestamp','description','p1','p2','p3','p4','p5','p6','p7','p8','p9','p10','p11','p12') UNION (select timestamp, description,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12 from $mysql_table where timestamp between $start and $end into outfile '/tmp/$mysql_table.csv' fields terminated by ',' lines terminated by '\r\n')"
	...
	echo node7,p7 >> $j/$i/mapping.csv
	echo node8,p8 >> $j/$i/mapping.csv
	echo node9,p9 >> $j/$i/mapping.csv
	echo node10,p10 >> $j/$i/mapping.csv
	echo node11,p11 >> $j/$i/mapping.csv
	echo node12,p12 >> $j/$i/mapping.csv
